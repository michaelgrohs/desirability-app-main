...
class Parser:
    def __init__(self, bpmn, is_file, transitivity, sanitize=False) -> None:
        self.transitivity = transitivity
        self.bpmn_model = self.__create_model(bpmn, is_file)
        self.bpmn_diagram = self.bpmn_model
        self.sequence = []
        self.is_json = True
        self.sanitizer = Sanitizer(sanitize)
        self.model = None
    ...
    def __find_transitive_closure(self, cfo, transitivity, toplevel, deep):
        if not deep and cfo == toplevel:
            cfo = None
        if cfo:
                ...
                if successor:
                    ...
                    for successor in cfo.get("successor"):
                        ...
                        self.__find_transitive_closure(
                            successor_cfo, transitivity, toplevel, True
                        )

    def __add_transitivity(self):
        for cfo in self.sequence:
            transitivity = []
            self.__find_transitive_closure(cfo, transitivity, cfo, False)
            if transitivity:
