...
const CriticalityResults: React.FC = () => {
  ...
  const criticalityMap: CriticalityMap =
    location.state?.criticalityMap || {};
  ...
  const criticalityWeight = (label: string | null) => {
      switch (label) {
        case 'very negative':
          return 3;
        case 'negative':
  ...
  const deviationPriorities = deviations.map(dev => {
      let score = 0;
      let reasons: string[] = [];
    ...
    <Box sx={{ width: '90vw', maxWidth: 1100, margin: '0 auto', mt: 4 }}>
      <Box display="flex" justifyContent="space-between" mb={4}>
        ...
        <Box display="flex" gap={3} mb={3} alignItems="center">
          ...
          {[
            { label: 'very positive', color: 'rgba(0,100,0,0.8)' },
            { label: 'positive', color: 'rgba(76,175,80,0.7)' },
            { label: 'negative', color: 'rgba(255,152,0,0.7)' },
            { label: 'very negative', color: 'rgba(211,47,47,0.8)' }
          ].map(item => (
        ...
        <Button variant="outlined" onClick={exportCSV} sx={{ mt: 3 }}>
          Export as CSV
        </Button>
        <Button variant="contained" sx={{ ml: 2 }} onClick={exportPDF}>
          Export as PDF
        </Button>
